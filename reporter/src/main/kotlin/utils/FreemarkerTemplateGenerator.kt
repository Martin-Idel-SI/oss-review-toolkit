...
class FreemarkerTemplateGenerator(
    private val filePrefix: String,
    private val defaultTemplateName: String
) {
    companion object {
    ...
    fun generateTemplate(input: ReporterInput, outputDir: File, options: Map<String, String>): List<File> {
        ...
        val freemarkerConfig = Configuration(Configuration.VERSION_2_3_30).apply {
            ...
            templateLoader = ClassTemplateLoader(
                this@FreemarkerTemplateGenerator.javaClass.classLoader,
                "template.notice"
            )
        ...
        val templateIds = options[OPTION_TEMPLATE_ID]?.split(",")
            ?: if (templatePaths.isEmpty()) listOf(defaultTemplateName) else emptyList()
